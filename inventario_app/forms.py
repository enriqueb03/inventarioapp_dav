...
class ProductoForm(forms.ModelForm):
    class Meta:
        model = Producto
        fields = ['sku', 'imagen',  'nombre', 'categoria',
                  'cantidad', 'precio_compra', 'precio_venta', 'activo']
...
# Asegúrate de tener esta importación en la parte de arriba de tu forms.py

# ... (Aquí arriba están los formularios que ya tenías) ...


class CrearUsuarioForm(forms.ModelForm):
    password = forms.CharField(widget=forms.PasswordInput(
        attrs={'class': 'w-full px-4 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500'}), label="Contraseña")
    ...
    class Meta:
        ...
        widgets = {
            'username': forms.TextInput(attrs={'class': 'w-full px-4 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500'}),
            'email': forms.EmailInput(attrs={'class': 'w-full px-4 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500'}),
        }
